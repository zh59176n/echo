...
const SCAN_STEPS = [
  ...
  'Checking data broker databases…',
  'Analyzing username cross-linking…',
  'Detecting email exposures…',
...
function buildClientMock(username, email, website) {
  return {
    ...
    exposure_risks: [
      ...
      { id: 'username_link', title: 'Username linked across platforms', description: `Username "${username}" found on 4 platforms, enabling cross-profiling`, severity: 'medium' },
      ...
      { id: 'data_broker', title: 'Listed on data broker sites', description: 'Personal information aggregated by 2 data brokers', severity: 'medium' },
    ],
    recommended_actions: [
    ...
    views: {
      ...
      threat: {
        ...
        summary: `The exposed email and consistent username for ${username} create targetable attack vectors for phishing and credential stuffing. Cross-platform linking allows a threat actor to build a detailed profile with minimal effort. No breach data was found, which significantly reduces immediate risk.`,
      },
    },
  }
}
...
export default function Landing({ onGenerate }) {
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      const full = { ...buildClientMock(username, email, website), ...data }
      onGenerate(full)
    } catch {
      onGenerate(buildClientMock(username, email, website))
    } finally {
      setLoading(false)
      setStep(-1)
    }
      ...
      {loading && (
        <div className="loading-overlay">
          <div className="loading-box">
            ...
            <div className="loading-title">Scanning your digital footprint</div>
            <div className="loading-sub">Analyzing public sources — this takes just a moment.</div>
